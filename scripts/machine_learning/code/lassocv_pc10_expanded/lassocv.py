...
def main(input_path, target, output_file_path_results_per_fold, output_file_path_test_aggregated, random_state=42, folds_str='folds_r0', PF=None):
    ...
    model_str = 'LassoCV_pc10_expanded'
    ...
    for fold, (train_index, test_index) in enumerate(zip(train_indexes, test_indexes)):
        ...
        top_corr_features, _ = evaluate_correlation(df_train, numeric_columns_non_zero_pad_filtered, target, fraction_or_N=0.10)


        model, alpha, selected_features, y_train_pred, y_test_pred = simple_lassocv(df_train, df_test, top_corr_features, target)
