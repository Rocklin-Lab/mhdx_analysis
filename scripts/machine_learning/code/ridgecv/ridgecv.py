...
def main(input_path, target, output_file_path_results_per_fold, output_file_path_test_aggregated, random_state=42, folds_str='folds_r0', PF=None):
    ...
    for fold, (train_index, test_index) in enumerate(zip(train_indexes, test_indexes)):
        ...
        model, alpha, selected_features, y_train_pred, y_test_pred = simple_ridgecv(df_train, df_test, numeric_columns_non_zero_pad, target)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target', type=str, required=True, help='Target value: dg_mean, normalized_cooperativity_model_global or normalized_cooperativity_model_pf')
